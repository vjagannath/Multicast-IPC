...
int (* const call_vec[NR_PM_CALLS])(void) = {
	...
	CALL(PM_MSEND)		= do_msend,			/* msend(2) */
	CALL(PM_MRECEIVE)	= do_mreceive,		/* mreceive(2) */
	CALL(PM_OPENGROUP)	= do_opengroup,		/* opengroup(2) */
	CALL(PM_CLOSEGROUP)	= do_closegroup,		/* closegroup(2) */
	CALL(PM_RECOVERGROUP)	= do_recovergroup,	/* recovergroup(2) */
	CALL(PM_GETGROUPINFO)	= do_getgroupinfo	/* getgroupinfo(2) */
};
